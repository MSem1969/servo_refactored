      ...
      <div className="px-4 py-2 border-b border-slate-200 bg-slate-50 shrink-0">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
            ...
            <h3 className="text-sm font-semibold text-slate-800 truncate">{ticketData.oggetto}</h3>
          </div>
          <select
            value={ticketData.stato}
            onChange={(e) => handleStatusChange(e.target.value)}
            disabled={updateStatusMutation.isPending}
            className="px-3 py-1 border border-slate-200 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 shrink-0"
          >
            ...
            <div className="flex items-center gap-1">
              {allegatiList.map(allegato => (
                <button
                  ...
                  className="flex items-center gap-1 px-1.5 py-0.5 bg-white hover:bg-slate-100 rounded text-xs text-slate-600 border border-slate-200"
                  title={allegato.nome_originale}
                >
                  <span>{allegato.mime_type?.startsWith('image/') ? '🖼️' : '📄'}</span>
                  <span className="truncate max-w-[100px]">{allegato.nome_originale}</span>
                </button>
              ))}
            </div>
          )}
