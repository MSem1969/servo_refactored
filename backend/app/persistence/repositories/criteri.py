# =============================================================================
# SERV.O v10.1 - CRITERI ORDINARI REPOSITORY
# =============================================================================
# Repository per gestione criteri ML (pattern ordinari)
# =============================================================================

from typing import Dict, Any, List, Optional
from .base import BaseRepository


class CriteriEspositoreRepository(BaseRepository[Dict[str, Any]]):
    """Repository per criteri_ordinari_espositore."""

    def __init__(self):
        super().__init__('criteri_ordinari_espositore', 'pattern_signature')

    def get_by_pattern(self, pattern_signature: str) -> Optional[Dict[str, Any]]:
        """Recupera criterio per pattern signature."""
        return self._execute_one(
            "SELECT * FROM criteri_ordinari_espositore WHERE pattern_signature = %s",
            (pattern_signature,)
        )

    def get_ordinari(self) -> List[Dict[str, Any]]:
        """Lista criteri promossi a ordinari."""
        return self._execute_query(
            "SELECT * FROM criteri_ordinari_espositore WHERE is_ordinario = TRUE ORDER BY data_promozione DESC"
        )

    def get_by_vendor(self, vendor: str) -> List[Dict[str, Any]]:
        """Lista criteri per vendor."""
        return self._execute_query(
            "SELECT * FROM criteri_ordinari_espositore WHERE vendor = %s ORDER BY count_approvazioni DESC",
            (vendor,)
        )

    def increment_approvazioni(self, pattern_signature: str, operatore: str) -> bool:
        """Incrementa contatore approvazioni."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_espositore
            SET count_approvazioni = count_approvazioni + 1,
                operatori_approvatori = COALESCE(operatori_approvatori || ',', '') || %s
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (operatore, pattern_signature)).fetchone()
        db.commit()
        return result is not None

    def promuovi_a_ordinario(self, pattern_signature: str) -> bool:
        """Promuove pattern a ordinario."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_espositore
            SET is_ordinario = TRUE, data_promozione = NOW()
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (pattern_signature,)).fetchone()
        db.commit()
        return result is not None

    def reset_pattern(self, pattern_signature: str) -> bool:
        """Reset pattern (non più ordinario)."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_espositore
            SET is_ordinario = FALSE, count_approvazioni = 0
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (pattern_signature,)).fetchone()
        db.commit()
        return result is not None

    def get_stats(self) -> Dict[str, Any]:
        """Statistiche criteri espositore."""
        return {
            'totale': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_espositore"),
            'ordinari': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_espositore WHERE is_ordinario = TRUE"),
            'in_apprendimento': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_espositore WHERE is_ordinario = FALSE"),
        }


class CriteriListinoRepository(BaseRepository[Dict[str, Any]]):
    """Repository per criteri_ordinari_listino."""

    def __init__(self):
        super().__init__('criteri_ordinari_listino', 'pattern_signature')

    def get_by_pattern(self, pattern_signature: str) -> Optional[Dict[str, Any]]:
        """Recupera criterio per pattern signature."""
        return self._execute_one(
            "SELECT * FROM criteri_ordinari_listino WHERE pattern_signature = %s",
            (pattern_signature,)
        )

    def get_ordinari(self) -> List[Dict[str, Any]]:
        """Lista criteri promossi a ordinari."""
        return self._execute_query(
            "SELECT * FROM criteri_ordinari_listino WHERE is_ordinario = TRUE ORDER BY data_promozione DESC"
        )

    def get_by_aic(self, codice_aic: str) -> List[Dict[str, Any]]:
        """Lista criteri per codice AIC."""
        return self._execute_query(
            "SELECT * FROM criteri_ordinari_listino WHERE codice_aic = %s",
            (codice_aic,)
        )

    def increment_approvazioni(self, pattern_signature: str, operatore: str) -> bool:
        """Incrementa contatore approvazioni."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_listino
            SET count_approvazioni = count_approvazioni + 1,
                operatori_approvatori = COALESCE(operatori_approvatori || ',', '') || %s
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (operatore, pattern_signature)).fetchone()
        db.commit()
        return result is not None

    def promuovi_a_ordinario(self, pattern_signature: str) -> bool:
        """Promuove pattern a ordinario."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_listino
            SET is_ordinario = TRUE, data_promozione = NOW()
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (pattern_signature,)).fetchone()
        db.commit()
        return result is not None

    def get_stats(self) -> Dict[str, Any]:
        """Statistiche criteri listino."""
        return {
            'totale': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_listino"),
            'ordinari': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_listino WHERE is_ordinario = TRUE"),
            'in_apprendimento': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_listino WHERE is_ordinario = FALSE"),
        }


class CriteriLookupRepository(BaseRepository[Dict[str, Any]]):
    """Repository per criteri_ordinari_lookup."""

    def __init__(self):
        super().__init__('criteri_ordinari_lookup', 'pattern_signature')

    def get_by_pattern(self, pattern_signature: str) -> Optional[Dict[str, Any]]:
        """Recupera criterio per pattern signature."""
        return self._execute_one(
            "SELECT * FROM criteri_ordinari_lookup WHERE pattern_signature = %s",
            (pattern_signature,)
        )

    def get_ordinari(self) -> List[Dict[str, Any]]:
        """Lista criteri promossi a ordinari."""
        return self._execute_query(
            "SELECT * FROM criteri_ordinari_lookup WHERE is_ordinario = TRUE ORDER BY data_promozione DESC"
        )

    def get_by_piva(self, partita_iva: str) -> List[Dict[str, Any]]:
        """Lista criteri per partita IVA pattern."""
        return self._execute_query(
            "SELECT * FROM criteri_ordinari_lookup WHERE partita_iva_pattern = %s",
            (partita_iva,)
        )

    def increment_approvazioni(self, pattern_signature: str, operatore: str) -> bool:
        """Incrementa contatore approvazioni."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_lookup
            SET count_approvazioni = count_approvazioni + 1,
                operatori_approvatori = COALESCE(operatori_approvatori || ',', '') || %s
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (operatore, pattern_signature)).fetchone()
        db.commit()
        return result is not None

    def promuovi_a_ordinario(self, pattern_signature: str) -> bool:
        """Promuove pattern a ordinario."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_lookup
            SET is_ordinario = TRUE, data_promozione = NOW()
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (pattern_signature,)).fetchone()
        db.commit()
        return result is not None

    def set_default_farmacia(self, pattern_signature: str, min_id: str, id_farmacia: int) -> bool:
        """Imposta farmacia default per pattern."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_lookup
            SET min_id_default = %s, id_farmacia_default = %s
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (min_id, id_farmacia, pattern_signature)).fetchone()
        db.commit()
        return result is not None

    def get_stats(self) -> Dict[str, Any]:
        """Statistiche criteri lookup."""
        return {
            'totale': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_lookup"),
            'ordinari': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_lookup WHERE is_ordinario = TRUE"),
            'in_apprendimento': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_lookup WHERE is_ordinario = FALSE"),
        }


class CriteriAicRepository(BaseRepository[Dict[str, Any]]):
    """Repository per criteri_ordinari_aic."""

    def __init__(self):
        super().__init__('criteri_ordinari_aic', 'pattern_signature')

    def get_by_pattern(self, pattern_signature: str) -> Optional[Dict[str, Any]]:
        """Recupera criterio per pattern signature."""
        return self._execute_one(
            "SELECT * FROM criteri_ordinari_aic WHERE pattern_signature = %s",
            (pattern_signature,)
        )

    def get_ordinari(self) -> List[Dict[str, Any]]:
        """Lista criteri promossi a ordinari."""
        return self._execute_query(
            "SELECT * FROM criteri_ordinari_aic WHERE is_ordinario = TRUE ORDER BY data_promozione DESC"
        )

    def get_by_vendor_descrizione(self, vendor: str, descrizione_normalizzata: str) -> Optional[Dict[str, Any]]:
        """Cerca criterio per vendor e descrizione normalizzata."""
        return self._execute_one(
            "SELECT * FROM criteri_ordinari_aic WHERE vendor = %s AND descrizione_normalizzata = %s",
            (vendor, descrizione_normalizzata)
        )

    def increment_approvazioni(self, pattern_signature: str, operatore: str) -> bool:
        """Incrementa contatore approvazioni."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_aic
            SET count_approvazioni = count_approvazioni + 1,
                operatori_approvatori = COALESCE(operatori_approvatori || ',', '') || %s
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (operatore, pattern_signature)).fetchone()
        db.commit()
        return result is not None

    def promuovi_a_ordinario(self, pattern_signature: str) -> bool:
        """Promuove pattern a ordinario."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_aic
            SET is_ordinario = TRUE, data_promozione = NOW()
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (pattern_signature,)).fetchone()
        db.commit()
        return result is not None

    def set_default_aic(self, pattern_signature: str, codice_aic: str) -> bool:
        """Imposta AIC default per pattern."""
        db = self._get_db()
        result = db.execute("""
            UPDATE criteri_ordinari_aic
            SET codice_aic_default = %s
            WHERE pattern_signature = %s
            RETURNING pattern_signature
        """, (codice_aic, pattern_signature)).fetchone()
        db.commit()
        return result is not None

    def get_stats(self) -> Dict[str, Any]:
        """Statistiche criteri AIC."""
        return {
            'totale': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_aic"),
            'ordinari': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_aic WHERE is_ordinario = TRUE"),
            'in_apprendimento': self._execute_scalar("SELECT COUNT(*) FROM criteri_ordinari_aic WHERE is_ordinario = FALSE"),
        }


# =============================================================================
# REPOSITORY INSTANCES (Singleton-like)
# =============================================================================

_criteri_espositore_repo = None
_criteri_listino_repo = None
_criteri_lookup_repo = None
_criteri_aic_repo = None


def get_criteri_espositore_repo() -> CriteriEspositoreRepository:
    """Get or create CriteriEspositoreRepository instance."""
    global _criteri_espositore_repo
    if _criteri_espositore_repo is None:
        _criteri_espositore_repo = CriteriEspositoreRepository()
    return _criteri_espositore_repo


def get_criteri_listino_repo() -> CriteriListinoRepository:
    """Get or create CriteriListinoRepository instance."""
    global _criteri_listino_repo
    if _criteri_listino_repo is None:
        _criteri_listino_repo = CriteriListinoRepository()
    return _criteri_listino_repo


def get_criteri_lookup_repo() -> CriteriLookupRepository:
    """Get or create CriteriLookupRepository instance."""
    global _criteri_lookup_repo
    if _criteri_lookup_repo is None:
        _criteri_lookup_repo = CriteriLookupRepository()
    return _criteri_lookup_repo


def get_criteri_aic_repo() -> CriteriAicRepository:
    """Get or create CriteriAicRepository instance."""
    global _criteri_aic_repo
    if _criteri_aic_repo is None:
        _criteri_aic_repo = CriteriAicRepository()
    return _criteri_aic_repo
