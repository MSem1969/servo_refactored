...
def _insert_order(
    ...
    if righe_senza_prezzo:
        for riga in righe_senza_prezzo:
            # Crea anomalia LST-A01 per ogni riga
            cursor = db.execute("""
                INSERT INTO anomalie
            ...
            """, (
                id_testata,
                riga['id_dettaglio'],
                f"Prezzo mancante per AIC {riga['codice_aic'] or 'N/D'} - {riga['descrizione'][:40] if riga['descrizione'] else 'N/D'}",
    ...
    # =========================================================================
    # CHECK AIC-A01: PRODOTTI SENZA CODICE AIC VALIDO (v9.0)
    # =========================================================================
    # Cerca righe senza codice AIC valido (9 cifre), escludendo child espositori
    righe_senza_aic = db.execute("""
        SELECT id_dettaglio, n_riga, codice_aic, codice_originale, descrizione
        FROM ordini_dettaglio
        WHERE id_testata = %s
          AND (codice_aic IS NULL OR codice_aic = '' OR LENGTH(codice_aic) != 9)
