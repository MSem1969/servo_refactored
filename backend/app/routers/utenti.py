# =============================================================================
# SERV.O v6.2 - ROUTER GESTIONE UTENTI
# =============================================================================
...
async def list_utenti(
    ...
    if current_user.ruolo == RuoloUtente.ADMIN:
        ...
        conditions.append("ruolo IN ('admin', 'superuser', 'supervisore', 'operatore', 'readonly')")
    elif current_user.ruolo == RuoloUtente.SUPERUSER:
        # Superuser vede supervisori e operatori
        conditions.append("ruolo IN ('supervisore', 'operatore', 'readonly')")
    else:
...
async def update_profilo(
    ...
    # v8.1: Aggiunta email al profilo personale
    if profilo_data.email is not None:
        old_values["email"] = target_user["email"]
        updates.append("email = %s")
        params.append(profilo_data.email)
...
async def update_utente(user_id: int, request: Request, update_data: ModificaUtenteRequest,
    ...
    if not is_self:
        if current_user.ruolo not in [RuoloUtente.ADMIN, RuoloUtente.SUPERUSER, RuoloUtente.SUPERVISORE]:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Non hai i permessi")
        # v6.2.1: Controlli per ruoli inferiori
        if current_user.ruolo == RuoloUtente.SUPERVISORE:
            ...
            if target_user["ruolo"] not in ("supervisore", "operatore", "readonly"):
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Non puoi modificare questo utente")
...
async def get_user_logs(user_id: int, action_category: Optional[str] = Query(None),
    ...
    if current_user.ruolo == RuoloUtente.SUPERUSER:
        is_own = (user_id == current_user.id_operatore)
        is_own_created = (target_user["ruolo"] in ("supervisore", "operatore") and target_user["created_by_operatore"] == current_user.id_operatore)
        if not (is_own or is_own_created):
