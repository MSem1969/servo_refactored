...
@router.post("/clienti/import")
async def import_clienti(
    file: UploadFile = File(..., description="File CSV clienti")
) -> Dict[str, Any]:
    """
    Importa anagrafica clienti da CSV.

    Formato atteso: CSV con separatore , o ;
    Colonne: AGCANA, AGRSO1, AGRSO2, AGINDI, AGCAP, AGLOCA, AGPROV,
             AGPIVA, AGMAIL, AGCATE, AGCFAR, AGCSTA, AGCPAG, AGTIDD, AGDRIF
    """
    if not file.filename:
        raise HTTPException(status_code=400, detail="Nome file mancante")
    ...
    try:
        content = await file.read()
        result = import_anagrafica_clienti(csv_content=content, fonte=file.filename)
...
async def anagrafica_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        # Aggiungi stats clienti
        stats['clienti'] = get_clienti_stats()
        return {
            "success": True,
            "data": stats
        }
...
async def clear_parafarmacie(
    ...
    Operazione irreversibile! Richiede confirm=true
    """
    if not confirm:
        raise HTTPException(
            status_code=400,
...
@router.delete("/clienti")
async def clear_clienti(
    confirm: bool = Query(False, description="Conferma eliminazione")
) -> Dict[str, Any]:
    """
    Elimina tutti i clienti dall'anagrafica.
    ...
    try:
        count = clear_anagrafica_clienti()
        return {
            "success": True,
