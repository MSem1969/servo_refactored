...
# Path al modulo mail_monitor
MAIL_MONITOR_PATH = Path(__file__).parent.parent.parent.parent / "mail_monitor"
...
def _get_mail_config() -> Dict[str, Any]:
    ...
    """
    try:
        # Prova a importare la config dal modulo mail_monitor
        sys.path.insert(0, str(MAIL_MONITOR_PATH))
        ...
        return {
            ...
            "email": Config.MAIL_USER or "(non configurato)",
        ...
        # Rimuovi dal path
        if str(MAIL_MONITOR_PATH) in sys.path:
            sys.path.remove(str(MAIL_MONITOR_PATH))
