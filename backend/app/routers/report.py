# =============================================================================
# SERV.O v8.1 - ROUTER REPORT ESPORTAZIONE
# =============================================================================
...
async def get_report_data(
    ...
    """
    Genera dati report consolidato ordini.
...
async def get_stati(
    ...
    """Ritorna lista stati filtrati in base alle selezioni correnti."""
    db = get_db()

    conditions, params = build_cascade_conditions(
        data_inizio, data_fine, vendors, None, clienti, aic, tipo_data, depositi
    )
    ...
    if aic:
        query = f"""
            SELECT DISTINCT t.stato
            FROM v_ordini_completi t
            JOIN ordini_dettaglio d ON t.id_testata = d.id_testata
            {where_clause}
            AND t.stato IS NOT NULL
            ORDER BY t.stato
        """
    else:
        query = f"""
            ...
            FROM v_ordini_completi t
            {where_clause}
            {"AND" if conditions else "WHERE"} stato IS NOT NULL
            ORDER BY stato
        """

    rows = db.execute(query, tuple(params)).fetchall()
    return {'stati': [r['stato'] for r in rows]}
