...
def _log_access_attempt(
    ...
    try:
        db.execute(
            ...
            ) VALUES (%s, 'AUTH', 'operatore', %s, %s, %s, %s, %s, %s, %s, %s)
            """,
            (
                action,
                user_id,
...
async def require_admin_or_supervisor(
    ...
    """
    Dependency che richiede ruolo admin, superuser o supervisore.

    Uso comune per endpoint gestione utenti dove tutti hanno accesso
    (con scope diversi gestiti nel router).

    Raises:
        HTTPException 403: se né admin né superuser né supervisore

    Esempio:
        @router.get("/utenti")
        async def list_users(user: UtenteResponse = Depends(require_admin_or_supervisor)):
            # Admin vede tutti, superuser vede supervisori+operatori, supervisore vede solo operatori
            ...
    """
