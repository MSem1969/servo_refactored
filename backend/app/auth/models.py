...
class RuoloUtente(str, Enum):
    ...
    2. superuser   - Come supervisore, ma può creare supervisori e vede produttività supervisori
    3. supervisore - Frontend completo, crea operatori
    4. operatore   - Solo Database + Upload
    5. readonly    - Solo visualizzazione (legacy, mantenuto per compatibilità)
...
class ProfiloUpdateRequest(BaseModel):
    ...
    nome: Optional[str] = Field(None, min_length=2, max_length=100)
    cognome: Optional[str] = Field(None, min_length=2, max_length=100)
    email: Optional[EmailStr] = Field(None, description="Indirizzo email")
...
# =============================================================================
# MODELLI PASSWORD RESET (v8.2)
# =============================================================================

class ForgotPasswordRequest(BaseModel):
    """
    Request body per POST /auth/forgot-password.

    Invia email con link per reset password se l'email esiste nel sistema.
    Per sicurezza, ritorna sempre successo (non rivela se email esiste).
    """
    email: EmailStr = Field(..., description="Email associata all'account")


class ForgotPasswordResponse(BaseModel):
    """
    Response per forgot-password.

    Sempre successo per sicurezza (non rivela se email esiste).
    """
    message: str = "Se l'email esiste nel sistema, riceverai un link per reimpostare la password."
    success: bool = True


class ResetPasswordRequest(BaseModel):
    """
    Request body per POST /auth/reset-password.

    Richiede token valido (ricevuto via email) e nuova password.
    """
    token: str = Field(..., description="Token di reset ricevuto via email")
    nuova_password: str = Field(..., min_length=6, description="Nuova password (minimo 6 caratteri)")
...
class ResetPasswordResponse(BaseModel):
    """Response per reset-password riuscito."""
    message: str = "Password reimpostata con successo. Puoi ora accedere con la nuova password."
    success: bool = True
