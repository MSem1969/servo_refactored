...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(Config.LOGS_DIR / 'mail_monitor.log'),
...
def main():
    logger.info("=" * 60)
    logger.info("MAIL MONITOR - Avvio")
    logger.info("=" * 60)
    ...
    if not mail.connetti():
        logger.error("Impossibile connettersi al server mail")
        return 1

    try:
        if not mail.seleziona_cartella():
        ...
        for uid in uids:
            try:
                ...
                # Scarica email
                email_data = mail.scarica_email(uid)
                if not email_data:
