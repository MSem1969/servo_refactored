...
GMAIL_MONITOR_SCRIPT = SCRIPT_DIR / 'gmail_monitor.py'
...
print("Gmail Monitor Scheduler - Avviato")
...
while True:
    ...
    print(f"{now.strftime('%Y-%m-%d %H:%M:%S')} - Controllo Gmail...")
    try:
        subprocess.run([PYTHON_PATH, str(GMAIL_MONITOR_SCRIPT)], check=True)
        print("OK")
